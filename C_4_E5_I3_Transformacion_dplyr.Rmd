...
df%>% 
  select(value) %>% 
  mutate(Change_df=value-lag(value, 7))
...
# Claculate the change in value from one (1) time period and four (4) time periods
df%>% 
  select(value) %>% 
  mutate(lead1=value-lead(value)) %>% 
  mutate(lead4=value-lead(value, 4))
```
...
df %>% 
  select(value) %>% 
  mutate(porcentaje=percent_rank(value)) # equivalente a rank, Un número entre a  0 y 1 calculado haciendo una re-escala de los valores usando min_rank to [0, 1]. NOTA que el valor más pequeño es reasignado un valor de 0 y el más grande un valor de 1.  
...
Es la suma cumulativba de los rangos
...
# summarise() by group using group_by()
